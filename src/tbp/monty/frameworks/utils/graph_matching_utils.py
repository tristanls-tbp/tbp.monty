...
def get_correct_k_n(k_n, num_datapoints):
    ...
    number of data points collected, so this function checks and adjusts k_n.
...
def get_unique_paths(possible_paths, threshold=0.01):
    ...
    Args:
        ...
        threshold: Minimum distance between two paths to be considered
...
def get_uniform_initial_possible_poses(n_degrees_sampled=9):
    ...
    Args:
        n_degrees_sampled: Number of degrees sampled for each axis. Defaults to 9,
            which means tested degrees are in [0., 45., 90., 135., 180., 225.,
            270., 315.]. This results in 512 unique pose combinations.

    Depending on the displacement vector, some poses are equivalent
    (e.g., [0, 0, 0] and [180, 180, 180] or [0, 45, 90] and [180, 135, 270]).
    (a, b, c) == (a + 180, -b + 180, c + 180) (see
    https://books.google.gr/books?id=rn3OBQAAQBAJ p.267).
...
def process_delta_evidence_values(max_ev_per_step):
    ...
    Clip the values to be less than or equal to 0 and return the index of the most
    recent positive change in evidence.
    ...
    Returns:
        ...
        positive_jump_loc: Index of the most recent positive change in evidence.
    """
