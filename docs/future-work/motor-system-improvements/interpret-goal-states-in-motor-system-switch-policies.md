...
For example, we might want to do a semi-random walk when the motor system is not receiving goal states. We may want to switch to a saccade policy (rotating the sensor without moving it's position) when the sensor module sends a goal based on detected salient features.
