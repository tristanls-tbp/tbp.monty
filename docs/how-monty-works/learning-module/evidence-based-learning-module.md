...
[2025/02 - Deep Dive on the EvidenceLM](https://www.youtube.com/watch?v=VKWhQJzNpks)
...
The basic approach initializes the full hypothesis space once at the beginning of an episode and maintains these hypotheses throughout the remainder of the episode. This works well when the agent observes a single object throughout the episode, but it cannot adapt to changes in what it is sensing. Examples of this include when the agent moves from one object to another in an environment, or from one child object to another within a compositional model. In these cases the existing hypotheses, which were sampled for the previous object, no longer match the incoming observations, and new hypotheses would need to be initialized.

Ideally, we want to move away from episode-based inference entirely (see [RFC 9](https://github.com/thousandbrainsproject/tbp.monty/blob/main/rfcs/0009_hypotheses_resampling.md) and [RFC 13](https://github.com/thousandbrainsproject/tbp.monty/blob/main/rfcs/0013_resampling_dynamic_adjustment.md)) and allow Monty to continuously infer objects and switch between them without explicit resets. This is important for [unsupervised inference](../../overview/benchmark-experiments.md#unsupervised-inference) where objects are swapped without resetting Monty's internal state, for moving between child objects in a compositional model, and for inferring different morphologies at different keyframes in a behavior (e.g., an open versus a closed stapler).

The burst sampling hypotheses updater is a step toward this goal. It dynamically adds and removes hypotheses based on how well they explain the incoming observations. When the agent transitions to a new object, existing hypotheses result in poor predictions, triggering a burst that samples new hypotheses informed by the new observations. The outdated hypotheses are quickly deleted as they cease to accumulate evidence, while some of the newly sampled hypotheses rapidly accumulate evidence for the current object.

The idea is to use the rate of change of evidence, the evidence slope, as a signal for managing the hypothesis space. An evidence slope tracker maintains a sliding window of recent evidence values for each hypothesis and computes the average slope over the window. Each hypothesis also has an age counter; hypotheses must reach a minimum age before they can be considered for removal, preventing newly added hypotheses from being deleted before they have had enough observations to accumulate meaningful evidence.

At each step, the updater checks the maximum global slope, which is the highest evidence slope across all hypotheses for all objects. If this value falls below the `burst_trigger_slope` parameter, it indicates that no current hypothesis is explaining the observations well, and a sampling burst is triggered. A burst lasts for `sampling_burst_duration` consecutive steps, during each of which new hypotheses are sampled and added to the hypothesis space for every object in memory. Spreading the burst over multiple steps rather than sampling all hypotheses based on a single observation helps reduce the effect of sensor noise. At every step, hypotheses whose smoothed evidence slope falls below the `deletion_trigger_slope` are removed, continuously refining the hypothesis space.

![An unsupervised inference experiment where three objects (potted meat can, dice, mug) are shown to Monty and swapped without Monty's knowledge. The bottom plot shows the maximum recent evidence change (solid blue, left axis) and the hypothesis space size (dashed blue, right axis) over time. Dashed purple vertical lines indicate sampling bursts triggered when the evidence slope drops after an object swap. The hypothesis space size grows during bursts as new hypotheses are added, then shrinks as unlikely hypotheses are continuously deleted.](../../figures/how-monty-works/burst_sampling.gif)
