...
| **JumpToGoalStateMixin**           | This is a policy that immediately moves to a specified goal, and is implemented as a mixin, such that it can be used in combination with a variety of other core policies. This is currently the only way for LM goals (such as those generated by `EvidenceGoalStateGenerator`, and the associated "hypothesis-testing policy") to be enacted. |
